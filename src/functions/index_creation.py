...
def get_aoss_host(resource_properties):
    """
    Extract and sanitize the AOSS host from resource properties.
    Example: 'abc123.region.aoss.amazonaws.com'
    """
    if "AOSSHost" not in resource_properties:
        raise Exception("AOSSHost not provided in resource properties")

    raw_host = resource_properties["AOSSHost"]
    print("loooook  here eya"+raw_host)
    # Remove any accidental protocol or slashes
    sanitized_host = raw_host.replace("https://", "").split("/")[0]
    print("loooook also her"+sanitized_host)
